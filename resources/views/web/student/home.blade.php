...
@section('content')
	<section class="section-home">
		<div style="display: flex; flex-direction: row; justify-content: space-between; align-items: center; flex-wrap: true">
			...
			@if ($lastEnrollmentPeriod->end > date('Y-m-d'))
				...
				@if ($lastEnrollment->semestre === $lastEnrollmentPeriod->semestre)
					...
					<img class="blinking" style="position: absolute;top:55px; right: 165px;" src="{{ asset('img/seta-para-baixo.png') }}"
						alt="seta">
				@else
					<a id="novaInscricaoLink" class="novaInscricaoLink" href="{{ route('enrollment-store') }}" style="">Nova
						Inscrição</a>
				@endif
				{{-- @else
					<p
			...
			<form id="enrollmentform" style="display: none" action="{{ route('enrollment-store') }}" method="post"
				...
				<div class="flex w-[40rem] flex-col gap-2 rounded-md bg-white">
					...
					<div class="mb-2 flex w-full flex-col gap-2 border-2 p-2 px-4">
						<span>Serviços</span>
						<select name="taxa" id="taxa" class="h-8 w-full rounded-md border-2 border-blue-400 outline-none"
					...
					<div class="mb-2 flex w-full flex-col gap-2 border-2 p-2 px-4">
						...
						<input type="number" name="number" class="h-8 w-full rounded-md border-2 border-blue-400 px-2 outline-none"
